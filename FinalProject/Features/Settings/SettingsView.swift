...
struct SettingsView: View {
        ...
        NavigationStack {
            Form {
                ...
                Section(header: Text("Pomodoro Timer")) {
                    stepperRow(label: "Focus Duration",  value: $store.config.workMinutes,
                               range: 5...90,  step: 5, unit: "min")
                    stepperRow(label: "Short Break",     value: $store.config.shortBreakMinutes,
                               range: 1...30,  step: 1, unit: "min")
                    stepperRow(label: "Long Break",      value: $store.config.longBreakMinutes,
                               range: 5...60,  step: 5, unit: "min")
                    stepperRow(label: "Long Break After", value: $store.config.longBreakAfterPomodoros,
                               range: 2...8,   step: 1, unit: "pomodoros")
                ...
                Section(header: Text("Daily Goal")) {
                    ...
                    Slider(value: $store.config.dailyGoalHours, in: 0.5...12, step: 0.5)
                        .tint(StudyTheme.accent)
                }
                .listRowBackground(StudyTheme.surface)
    ...
    private func stepperRow(label: String, value: Binding<Int>,
    ...
    private func infoRow(icon: String, label: String, value: String) -> some View {
        HStack {
            Image(systemName: icon).foregroundStyle(StudyTheme.accent).frame(width: 24)
            Text(label)
            Spacer()
            Text(value).foregroundStyle(StudyTheme.secondaryText)
        }
    }
}
